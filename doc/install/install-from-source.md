...
First, follow our [github](https://github.com/LiuGroupHNU/nvnmd) webpage to download training and testing code for NVNMD.
...
```
It is notice that everytime a new shell is started and one wants to use `DeePMD-kit` and `NVNMD`, the virtual environment should be activated by 
```bash
source $tensorflow_venv/bin/activate
```
...
```
One should remember to activate the virtual environment every time he/she uses DeePMD-kit and NVNMD.

### Install the NVNMD's python interface
...
If you don't install horovod, DeePMD-kit and NVNMD will fallback to serial mode.
...
It is noticed that the I-Pi support is only compiled with gcc >= 4.8.
...
### Install the NVNMD's C++ interface
...
cd build
```
I assume you want to install NVNMD into path `$nvnmd_root`, then execute cmake
```bash
cmake -DTENSORFLOW_ROOT=$tensorflow_root -DCMAKE_INSTALL_PREFIX=$nvnmd_root ..
```
...
| -DCMAKE_INSTALL_PREFIX=&lt;value&gt; | Path          | -             | The Path where NVNMD will be installed. |
...
If everything works fine, you will have the following executable and libraries installed in `$nvnmd_root/bin` and `$nvnmd_root/lib`
```bash
$ ls $nvnmd_root/bin
dp_ipi      dp_ipi_low
$ ls $nvnpmd_root/lib
