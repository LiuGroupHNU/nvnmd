...
from common import make_default_mesh
...
class DeepTensor(DeepEval) :
    ...
    def __init__(self, 
        ...
        ##MZ 20200718
        self.t_G = self.graph.get_tensor_by_name(os.path.join(load_prefix, 'o_G:0'))
        self.t_GR = self.graph.get_tensor_by_name(os.path.join(load_prefix, 'o_GR:0'))
        self.t_F = self.graph.get_tensor_by_name(os.path.join(load_prefix, 'o_F:0'))
        self.t_D = self.graph.get_tensor_by_name(os.path.join(load_prefix, 'o_descriptor:0'))
        self.t_isTraining = self.graph.get_tensor_by_name(os.path.join(load_prefix, 'isTraining:0'))
        # outputs
    ...
    def eval(self,
        ...
        feed_dict_test[self.t_isTraining  ] = False
        ##MZ 20200718
        tensor_G = []
        tensor_GR = []
        tensor_D = []
        tensor_F = []
        t_out = [self.t_tensor, self.t_G, self.t_GR, self.t_D, self.t_F]
        # t_out = [self.t_tensor]
        for ii in range(nframes) :
            ...
            tensor_G.append(v_out[1])
            tensor_GR.append(v_out[2])
            tensor_D.append(v_out[3])
            tensor_F.append(v_out[4])
        ...
        return tensor, tensor_G, tensor_GR, tensor_D, tensor_F
