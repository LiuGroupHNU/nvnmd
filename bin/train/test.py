...
from common import expand_sys_str
...
def test_ener (dp, args) :
    ...
    fe = open(detail_file+".e.out", "ab")
    ff = open(detail_file+".f.out", "ab")
    fv = open(detail_file+".v.out", "ab")
    if detail_file is not None :
...
def test_dipole (dp, args) :
    ...
    data.add('force', 3, atomic=True, must=True, high_prec=False, type_sel = dp.get_sel_type())
    test_data = data.get_test ()
    numb_test = args.numb_test
    natoms = len(test_data["type"][0])
    nframes = test_data["box"].shape[0]
    ...
    box = test_data["box"][:numb_test]
    atype = test_data["type"][0]
    dipole, _, __, ___, ____ = dp.eval(coord, box, atype)

    dipole = dipole.reshape([numb_test,-1])
    l2f = (l2err (dipole  - test_data["force"] [:numb_test]))
    ...
    detail_file = args.detail_file
    if detail_file is not None :
        pe = np.concatenate((np.reshape(test_data["force"][:numb_test], [-1,3]),
