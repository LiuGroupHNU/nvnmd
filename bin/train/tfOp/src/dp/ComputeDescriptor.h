...
void compute_descriptor_se_a (vector<double > &			descrpt_a,
			       ...
			       const double &				rmin,
			       const double &				rmax,
             const double map_x1[2],
             const double map_k1[3],
             const double map_b1[3],
             const double map_x2[2],
             const double map_k2[3],
             const double map_b2[3],
             const double precs[3]);

inline
void compute_descriptor_se_a_q (vector<double > &			descrpt_a,
			       vector<double > &			descrpt_a_deriv,
			       vector<double > &			rij_a,
			       const vector<double > &			posi,
			       const int &				ntypes,
		...
		const TYPE & rmax) 
{
  ...
  if (xx < rmax) {
    double uu = (xx * xx - rmax*rmax) / (rmax * rmax) ;
    double du = 2. * xx / (rmax * rmax) ;
    vv = uu * uu;
    dd = 2 * uu * du;
  }
  else {
...
{  
  ...
  const double prec1 = precs[0]; // NBIT_DATA_FL
  const double prec2 = precs[1]; // NBIT_FEA_X
  const double prec3 = precs[2]; // NBIT_FEA_FL
  ...
  for (int sec_iter = 0; sec_iter < int(sec_a.size()) - 1; ++sec_iter){
    for (int nei_iter = sec_a[sec_iter]; nei_iter < sec_a[sec_iter+1]; ++nei_iter) {      
      ...
      //MZ r2u and u2r
      double rij[3];
      rij[0] = round(rr[0] * prec1) / prec1;
      rij[1] = round(rr[1] * prec1) / prec1;
      rij[2] = round(rr[2] * prec1) / prec1;
      double nr2 = MathUtilities::dot(rij, rij);
      //MZ r2u and u2r
      double tmp_r2, u, r2;
      r2 = floor(nr2 * prec1) / prec1;
      // R2U
      if       (r2 < map_x1[0]) u = r2 * map_k1[0] + map_b1[0];
      else {if (r2 < map_x1[1]) u = r2 * map_k1[1] + map_b1[1];
      else                      u = r2 * map_k1[2] + map_b1[2];}
      u = floor(u * prec2) / prec2;
      // U2R
      if       (u < map_x2[0]) r2 = u * map_k2[0] + map_b2[0];
      else {if (u < map_x2[1]) r2 = u * map_k2[1] + map_b2[1];
      else                     r2 = u * map_k2[2] + map_b2[2];}
      nr2 = r2 * rc2;
      //
      ...
      descrpt_a[idx_value + 1] = rij[0] / nr2;
      descrpt_a[idx_value + 2] = rij[1] / nr2;
      descrpt_a[idx_value + 3] = rij[2] / nr2;
      // deriv of component 1/r
      descrpt_a_deriv[idx_deriv + 0] = rij[0] * inr3 * sw - descrpt_a[idx_value + 0] * dsw * rij[0] * inr;
      descrpt_a_deriv[idx_deriv + 1] = rij[1] * inr3 * sw - descrpt_a[idx_value + 0] * dsw * rij[1] * inr;
      descrpt_a_deriv[idx_deriv + 2] = rij[2] * inr3 * sw - descrpt_a[idx_value + 0] * dsw * rij[2] * inr;
      // deriv of component x/r2
      descrpt_a_deriv[idx_deriv + 3] = (2. * rij[0] * rij[0] * inr4 - inr2) * sw - descrpt_a[idx_value + 1] * dsw * rij[0] * inr;
      descrpt_a_deriv[idx_deriv + 4] = (2. * rij[0] * rij[1] * inr4	      ) * sw - descrpt_a[idx_value + 1] * dsw * rij[1] * inr;
      descrpt_a_deriv[idx_deriv + 5] = (2. * rij[0] * rij[2] * inr4	      ) * sw - descrpt_a[idx_value + 1] * dsw * rij[2] * inr;
      // deriv of component y/r2
      descrpt_a_deriv[idx_deriv + 6] = (2. * rij[1] * rij[0] * inr4	      ) * sw - descrpt_a[idx_value + 2] * dsw * rij[0] * inr;
      descrpt_a_deriv[idx_deriv + 7] = (2. * rij[1] * rij[1] * inr4 - inr2) * sw - descrpt_a[idx_value + 2] * dsw * rij[1] * inr;
      descrpt_a_deriv[idx_deriv + 8] = (2. * rij[1] * rij[2] * inr4	      ) * sw - descrpt_a[idx_value + 2] * dsw * rij[2] * inr;
      // deriv of component z/r2
      descrpt_a_deriv[idx_deriv + 9] = (2. * rij[2] * rij[0] * inr4	      ) * sw - descrpt_a[idx_value + 3] * dsw * rij[0] * inr;
      descrpt_a_deriv[idx_deriv +10] = (2. * rij[2] * rij[1] * inr4	      ) * sw - descrpt_a[idx_value + 3] * dsw * rij[1] * inr;
      descrpt_a_deriv[idx_deriv +11] = (2. * rij[2] * rij[2] * inr4 - inr2) * sw - descrpt_a[idx_value + 3] * dsw * rij[2] * inr;
      // 4 value components
