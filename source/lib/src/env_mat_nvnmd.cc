...
{  
    ...
    mparams: x1[2], k1[3], b1[3], x2[2], k2[3], b2[3]
    */
   const double rc2 = rmax * rmax;
    ...
    for (int sec_iter = 0; sec_iter < int(sec_a.size()) - 1; ++sec_iter) {
        for (int nei_iter = sec_a[sec_iter]; nei_iter < sec_a[sec_iter+1]; ++nei_iter) {      
            ...
            double nr2 = deepmd::dot3(rij, rij);
            //r2u and u2r
            double tmp_r2, u, r2;
            r2 = floor(nr2 * precs[0]) / precs[0];
            // R2U
            if       (r2 < mparams[0]) u = r2 * mparams[2] + mparams[5];
            else {if (r2 < mparams[1]) u = r2 * mparams[3] + mparams[6];
            else                       u = r2 * mparams[4] + mparams[7];}
            u = floor(u * precs[1]) / precs[1];
            // U2R
            if       (u < mparams[8]) r2 = u * mparams[10] + mparams[13];
            else {if (u < mparams[9]) r2 = u * mparams[11] + mparams[14];
            else                      r2 = u * mparams[12] + mparams[15];}
            nr2 = r2 * rc2;
