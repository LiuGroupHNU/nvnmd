...
def filter_lower_R42GR(
  ...
  if (nvnmd_cfg.quantize_descriptor):
    ...
    with tf.variable_scope('rij', reuse=True):
        rij = op_module.quantize_nvnmd(rij, 0, -1, NBIT_DATA_FL, -1)
    # s, sr, G
    keys = 's,sr,G'.split(',')
    ...
    for ii in range(len(keys)):
        map_outs.append(op_module.map_nvnmd(u, map_tables[ii], map_tables2[ii]))
        with tf.variable_scope(keys[ii], reuse=True):
            map_outs[ii] = op_module.quantize_nvnmd(map_outs[ii], 0, -1, NBIT_DATA_FL, -1)
    s, sr, G = map_outs
    # R2R4
    Rs = s 
    Rxyz = sr * rij
