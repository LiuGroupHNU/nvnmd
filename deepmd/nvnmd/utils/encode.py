...
class Encode():
    ...
    def check_dec(self, idec, nbit, signed=False):
        ...
        if np.sum(I) > 0:
            ...
            print(f"{head}: there are data smaller than the lower limit {pmin}")
        if np.sum(I2) > 0:
            head = FioHead.warning()
            print(f"{head}: there are data bigger than the upper limit {pmax}")
    ...
    def dec2bin(self, idec, nbit=10, signed=False):
        idec = np.int64(np.reshape(np.array(idec), [-1]))
        self.check_dec(idec, nbit, signed)
